function hextohsl() {
  ...
  var test;
    ...
    var hexValues = sheet.getRange(firstRow, 1, lastRow - firstRow+1).getValues(); // get an array of the hex values
    ...
    var hslOutputRange = sheet.getRange(hslOutputRangeName);
  
    // loop over range
    for (var x = 0; x < (lastRow - firstRow + 1); x++) {
    
     hslValuesOutput.push( hexhslconvert(hexValues[x][0]) ); 
     // get HSL calculation back as an array and add it to the output array - setValues() and getValues() always use array of arrays ([[]]) as argument
    } 
